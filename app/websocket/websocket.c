 ...
 *         C/ Antonio Suarez N潞 10,
 *         Alcal谩 de Henares 28802 Madrid
 *         Spain
 *
 *      Email address:
...
noPollConn * conn1;
noPollMsg  * msg1;
LOCAL os_timer_t timerReceiveLisenter;

#define local_host_name		"47.97.252.149"
#define local_host_url		"/damdms/Steering/websocket/aabbccddeeff/machine"
#define local_host_port		"8083"

void timerReceiveLisenter_callback()
{
	if (! nopoll_conn_is_ok (conn1))
	{
		printf ("ERROR: Expected to find proper client connection status, but found error.. (conn=%p, conn->session=%d, NOPOLL_INVALID_SOCKET=%d, errno=%d, strerr=%s)..\n",
				conn1, (int) nopoll_conn_socket (conn1), (int) NOPOLL_INVALID_SOCKET, errno, strerror (errno));
	}
	else
	{
		if (nopoll_conn_send_text (conn1, "This is a test", 14) != 14)
		{
...
//	noPollMsg  * msg;
	...
	if (nopoll_ctx_conns (ctx1) != 0) {
		printf ("ERROR: expected to find 0 registered connections but found: %d\n", nopoll_ctx_conns (ctx1));
		return nopoll_false;
	} /* end if */

	nopoll_ctx_unref (ctx1);

	/* reinit again */
	ctx1 = create_ctx ();

	/* call to create a connection */
	conn1 = nopoll_conn_new (ctx1, local_host_name, local_host_port, NULL, local_host_url, NULL, NULL);
	if (! nopoll_conn_is_ok (conn1)) {
		printf ("ERROR: Expected to find proper client connection status, but found error.. (conn=%p, conn->session=%d, NOPOLL_INVALID_SOCKET=%d, errno=%d, strerr=%s)..\n",
			conn1, (int) nopoll_conn_socket (conn1), (int) NOPOLL_INVALID_SOCKET, errno, strerror (errno));
		return nopoll_false;
	...
	os_timer_arm(&timerReceiveLisenter, 10000, TRUE);
	...
	{
		iter = 0;
		while ((msg1 = nopoll_conn_get_msg (conn1)) == NULL) {
		...
		printf("msg=%s\n",(const char *) nopoll_msg_get_payload (msg1));
	}
...
//	char         buffer[1024];
//	int          bytes_read;
//	const char * msg = " klasdfkla akldfj klafklajetqkljt kjlwergklwejry90246tkgwr k帽ljwrglkjdfg lksdjglskg slkg cami贸n adsfasdf pru茅basdfad Espa帽a asdfaklsjdflk jasfkjaslfjetql tj帽qgkjadgklj aglkjalk jafkjaslfkjaskj asjaslfkjasfklajg klajef帽lqkjetrlkqj lqkj 帽lskdfja帽lk asldfj帽lafj a帽lfj 帽dfjkjt4帽qlkjt lkj34tlkja帽lgja帽lkgja帽lkgjw";
//
//	memset (buffer, 0, 1024);
//
//	/* reinit again */
...
//
//	/* call to create a connection */
//	conn = nopoll_conn_tls_new (ctx, opts, local_host_name, local_host_port, NULL, local_host_url, NULL, NULL);
//	if (! nopoll_conn_is_ok (conn)) {
//		printf ("ERROR: Expected to find proper client connection status, but found error..\n");
//		return nopoll_false;
//	}
...
//
//	/* call to create a connection */
//	conn = nopoll_conn_tls_new (ctx, opts, local_host_name, local_host_port, NULL, NULL, NULL, NULL);
//	if (! nopoll_conn_is_ok (conn)) {
//		printf ("ERROR: Expected to find proper client connection status, but found error..\n");
//		return nopoll_false;
//	} /* end if */
...
{
	int iterator = *argv;
	os_timer_disarm(&timerReceiveLisenter);//关闭定时器，相当于清零计时器计数
	os_timer_setfn(&timerReceiveLisenter, (os_timer_func_t *)timerReceiveLisenter_callback, NULL);//初始化定时器
